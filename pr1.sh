...
class ServerDeletionService
{
    ...
    public function handle(Server $server): void
    {
        ...
        // 🔒 Proteksi: hanya Risky yang boleh menghapus server siapa saja.
        ...
        if ($user) {
            if ($user->id !== 1) {
                ...
                if ($ownerId !== $user->id) {
                    throw new DisplayException('Jangan Del server orang ! Keamanan panel by riskyy.di');
                }
            }
...
echo "🔒 Hanya Risky yang bisa hapus server lain."
